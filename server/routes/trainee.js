...
const getFilteredTrainers = (trainers, noOfTrainer, trainerLocation) => {
  ...
  // Combine and balance male and female trainers
  const combinedTrainers = [...selectedExperiencedTrainers, ...selectedNewTrainers];
  const maleCount = Math.floor(combinedTrainers.length / 2);
  const femaleCount = combinedTrainers.length - maleCount;

  const maleTrainersFiltered = combinedTrainers.filter(trainer => trainer.gender === 'male').slice(0, maleCount);
  const femaleTrainersFiltered = combinedTrainers.filter(trainer => trainer.gender === 'female').slice(0, femaleCount);

  return [...maleTrainersFiltered, ...femaleTrainersFiltered];
};
...
router.post('/validate', verifyToken, async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < filteredTrainers.length; i++) {
        sendMail(filteredTrainers[i].email); // Access the name property
      }
